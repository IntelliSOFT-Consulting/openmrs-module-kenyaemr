...
public class CrossborderCohortLibrary {
	...
	public CohortDefinition crossborderClients() {
		Concept tbDiseaseStatus = Dictionary.getConcept(Dictionary.TUBERCULOSIS_DISEASE_STATUS);
		Concept diseaseSuspected = Dictionary.getConcept(Dictionary.DISEASE_SUSPECTED);
		Concept diseaseDiagnosed = Dictionary.getConcept(Dictionary.DISEASE_DIAGNOSED);
		Concept noSignsOrSymptoms = Dictionary.getConcept(Dictionary.NO_SIGNS_OR_SYMPTOMS_OF_DISEASE);
	...
	public CohortDefinition crossborderPatients() {
		...
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addSearch("otherNationality", ReportUtils.map(cbOtherNationalities(), "startDate=${onOrAfter},endDate=${onOrBefore}"));
		cd.addSearch("residentOtherCounty", ReportUtils.map(cbResidentOtherCounties(), "startDate=${onOrAfter},endDate=${onOrBefore}"));
		cd.addSearch("travelledOtherCountry", ReportUtils.map(cbTravelledOtherCountry(), "startDate=${onOrAfter},endDate=${onOrBefore}"));
	...
	public CohortDefinition receivedHivTestResults() {
		String sqlQuery = ""
				...
				" WHERE " + 
				"	t.test_type IN(1,2) AND" + 
				"	t.`patient_given_result` = 'Yes' AND" + 
	...
	public CohortDefinition cbReceivedHivTestResults() {
		...
		cd.addSearch("receivedHivTestResults", ReportUtils.map(receivedHivTestResults(), "startDate=${onOrAfter},endDate=${onOrBefore}"));
	...
	public CohortDefinition initiatedOnTreatment() {
		String sqlQuery = ""
				...
				"	c.patient_id " + 
				"FROM " + 
				"	kenyaemr_etl.`etl_current_in_care` c" + 
				"INNER JOIN " + 
				"	patient p ON c.patient_id = p.patient_id" + 
				"WHERE " + 
				"	p.voided = 0 AND " + 
	...
	public CohortDefinition cbInitiatedOnTreatment() {
		...
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addSearch("crossborder", ReportUtils.map(crossborderPatients(), "startDate=${onOrAfter},endDate=${onOrBefore}"));
		cd.addSearch("initiatedTreatment", ReportUtils.map(initiatedOnTreatment(), "startDate=${onOrAfter},endDate=${onOrBefore}"));
	...
	public CohortDefinition testedHivPositive() {
		...
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.setEncounterTypeList(Collections.singletonList(MetadataUtils.existing(EncounterType.class, CommonMetadata._EncounterType.CROSS_BORDER)));
		return cd;
	}
