...
public class CrossborderIndicatorReportBuilder extends AbstractReportBuilder {
	...
	protected List<Mapped<DataSetDefinition>> buildDataSets(ReportDescriptor descriptor, ReportDefinition report) {
		return Arrays.asList(
				ReportUtils.map(createCrossborderDataset(), "startDate=${startDate},endDate=${endDate}")/*,
	...
	private DataSetDefinition createCrossborderDataset() {
		...
		EmrReportingUtils.addRow(dsd, "CB1", "Number of other nationalities accessing specified services", ReportUtils.map(crossborderIndicators.cbOtherNationalities(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB2", "Number of residents of other districts/counties accessing services at the border facility", ReportUtils.map(crossborderIndicators.cbResidentOtherCounties(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB3", "Number reporting travelled to another country last 3/6/12 months", ReportUtils.map(crossborderIndicators.cbTravelledOtherCountry(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB4", "TXT-HTS: # of individuals who received HIV testing services (HTS) and received their test results, disaggregated by HIV result", ReportUtils.map(crossborderIndicators.receivedHivTestResults(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB5", "TX-POS: # of individuals who tested HIV positive", ReportUtils.map(crossborderIndicators.testedHivPositive(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB6", "TX-New: # of individuals newly initiated on Treatment", ReportUtils.map(crossborderIndicators.initiatedOnTreatment(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB8", "TX-SV:  who returned for second visit" , ReportUtils.map(crossborderIndicators.returnVisits(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB9", "TX-CURR: of adults and children currently receiving ART", ReportUtils.map(crossborderIndicators.receivingTreatment(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB10", "TX-PLVS: % of ART patients with a viral load result documented in the medical record and/or laboratory information systems (LIS) within the past 12 months with a suppressed viral load (<400 copies/ml)", ReportUtils.map(crossborderIndicators.virallySuppressed(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB11-1", "% of adults and children known to be on treatment 1 months after initiation of ART", ReportUtils.map(crossborderIndicators.retainedOnTreatmentAfterOneMonth(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB11-3", "% of adults and children known to be on treatment 3 months after initiation of ART", ReportUtils.map(crossborderIndicators.retainedOnTreatmentAfter3Months(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB11-6", "% of adults and children known to be on treatment 6 months after initiation of ART", ReportUtils.map(crossborderIndicators.retainedOnTreatmentAfter6Months(), indParams), columns);
		EmrReportingUtils.addRow(dsd, "CB11-12", "% of adults and children known to be on treatment 12 months after initiation of ART", ReportUtils.map(crossborderIndicators.retainedOnTreatmentAfter12Months(), indParams), columns);
